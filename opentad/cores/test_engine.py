...
from trans_utils import reset_net, SOPMonitor, TwoSideNeuron
from opentad.models.bricks.conv import IF
from . import utils

def get_all_spike_stats(model):
    """Collect spike statistics from all IF neurons in the model."""
    spike_total = 0
    neuron_total = 0
...
def eval_one_epoch(
    ...
    Args:
        ...
        cfg: Config object with inference and post_processing settings.
        logger: Logger instance.
        args: Argument namespace. Uses args.monitor if available (defaults to False).
        rank: Current process rank.
        model_ema: Optional EMA model.
        use_amp: Whether to use automatic mixed precision.
        world_size: Total number of distributed processes.
        not_eval: If True, skip evaluation (only do inference).
    """

    # Monitor is optional: available in test.py but not in train.py
    use_monitor = getattr(args, "monitor", False)
    mon = SOPMonitor(model, type=2)
    if use_monitor:
        mon.enable()
    else:
        mon.disable()
    ...
    if hasattr(args, 'mode') and args.mode == 'ann2nn':
        utils.save_model(model=model, model_without_ddp=model)
...
def gather_ddp_results(world_size, result_dict, post_cfg):
    """Gather results from all distributed processes and apply NMS for sliding window."""
