...
def train_one_epoch(
    ...
    for iter_idx, data_dict in enumerate(train_loader):
        ...
        if hasattr(model.module, "backbone"):
            if not model.module.backbone.freeze_backbone:
                curr_backbone_lr = scheduler.get_last_lr()[0]
        curr_det_lr = scheduler.get_last_lr()[-1]

        # forward pass (use float32 for training stability with SNN components)
        with torch.cuda.amp.autocast(dtype=torch.float32, enabled=use_amp):
