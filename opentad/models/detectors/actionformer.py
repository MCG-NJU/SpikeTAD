...
class ActionFormer(SingleStageDetector):
    def __init__(
        ...
        # T controls ANN/SNN mode: T=0 for ANN, T>0 for SNN with T timesteps.
        # Set externally via set_model_timesteps() in the test script.
        self.T = T
    ...
    def forward_test(self, inputs, masks, metas=None, infer_cfg=None, **kwargs):
        if self.with_backbone:
            if self.T > 0:
                # SNN mode: run backbone T times and accumulate features
                x_acc = None
                for i in range(self.T):
                    inputs_clone = torch.clone(inputs)
                    x = self.backbone(inputs_clone)
                    x_acc = x if x_acc is None else x_acc + x
                x = x_acc
            else:
                x = self.backbone(inputs)
        else:
